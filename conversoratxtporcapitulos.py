...
def _font(family, size_px, bold=False, italic=False):
    ...
    return (family, max(CONFIG["fonts"]["min_px"], int(size_px)), " ".join(style_bits) if style_bits else "")
...
class FlatButton(tk.Canvas):
    def __init__(self, parent, text, command=None, bg=None, accent=None,
        ...
        self._hover = False; self._pressed = False; self._font = font or _font(CONFIG["fonts"]["family"],  max(CONFIG["fonts"]["min_px"],  int(12)))
    ...
    def _fit_font(self, w: int, h: int):
        ...
        try:
            ...
            return self._font
        min_size = int(CONFIG.get("fonts", {}).get("min_px", 10))
...
class ChaptersDropdown(tk.Toplevel):
    def __init__(self, parent, width=420, height=420, font=None,
        ...
        if font is None:
            ...
            font = _font(fam, max(CONFIG["fonts"]["min_px"], 12))
...
class MiausoftApp(tk.Tk):
    def __init__(self, files):
        ...
        fam = CONFIG["fonts"]["family"]
        self.fonts = {
            "title": _font(fam, rh(CONFIG["fonts"]["title"]["rel_px"]), bold=CONFIG["fonts"]["title"]["bold"],
                           italic=CONFIG["fonts"]["title"]["italic"]),
            "sub": _font(fam, rh(CONFIG["fonts"]["sub"]["rel_px"]), bold=CONFIG["fonts"]["sub"]["bold"]),
            ...
            "err": _font(fam, rh(CONFIG["fonts"]["err"]["rel_px"]), bold=CONFIG["fonts"]["err"]["bold"]),
            "preview": _font(fam, rh(CONFIG["fonts"]["preview"]["rel_px"]), bold=CONFIG["fonts"]["preview"]["bold"]),
        }

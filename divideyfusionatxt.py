...
def _font(family, size_px, *, bold=False, italic=False):
    ...
    return (family, max(int(CONFIG["fonts"]["min_px"]), int(size_px)),
...
class FlatButton(tk.Canvas):
    ...
    def _fit_font(self, w: int, h: int) -> tkfont.Font:
        ...
        """
        fam = CONFIG["fonts"]["family"] or CONFIG["fonts"]["fallback_family"]
...
class FadePopover(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.label = tk.Label(self, text="", bg=CONFIG["colors"]["app_bg"], fg=CONFIG["colors"]["text"],
                              font=_font(CONFIG['fonts']['family'], CONFIG['fonts']['min_px']))
...
class SlideButton(tk.Canvas):
    def __init__(self, parent, *, state="Unir", on_change=None, row_height_px=22):
        ...
        self._font = _font(CONFIG["fonts"]["family"], max(int(CONFIG["fonts"]["min_px"]), int(h * 0.42)))
...
class FilesTable(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        min_px = int(CONFIG["fonts"]["min_px"])
        row_px = max(min_px, int(parent.winfo_height() * float(CONFIG["fonts"]["row"]["rel_px"])))
        head_px = max(row_px, int(parent.winfo_height() * float(CONFIG["fonts"]["head"]["rel_px"])))
        font_row  = _font(fam, row_px,  bold=CONFIG["fonts"]["row"]["bold"])
        font_head = _font(fam, head_px, bold=CONFIG["fonts"]["head"]["bold"])
    ...
    def _show_parts_spin(self, iid: str, x: int, y: int, w: int, h: int):
        ...
        sb = tk.Spinbox(self, from_=2, to=99, increment=1,
                        font=(CONFIG["fonts"]["family"], int(CONFIG["fonts"]["min_px"])),
...
class App(tk.Tk):
    ...
    def _sync_shared_btn_font(self, bw: int, bh: int) -> None:
        ...
        try:
            ...
            fam_btn = (CONFIG["fonts"].get("family") or CONFIG["fonts"].get("fallback_family") or "Segoe UI")
            style = CONFIG["fonts"]["btn"]

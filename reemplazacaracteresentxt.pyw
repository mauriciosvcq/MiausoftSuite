...
def _font(family, size_px, *, bold=False, italic=False):
    ...
    return (family, max(int(CONFIG["utilities"]["min_px"]), int(size_px)), " ".join(style_bits) if style_bits else "")
...
class FlatButton(tk.Canvas):
    ...
    def _style_from_tuple(self, ft: tuple) -> tuple[str, int, str, str]:
        fam = str(ft[0]) if len(ft) > 0 else str(CONFIG["utilities"]["family"])
        size = int(ft[1]) if len(ft) > 1 else int(CONFIG["utilities"]["min_px"])
